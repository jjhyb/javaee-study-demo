...
//		request.getRequestDispatcher("/list.jsp").forward(request, response);
	 ...
	 * @throws IOException
	 */
	public void deleteChecked(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		String[] pids = request.getParameterValues("pid");
		System.out.println(pids);
		//调用业务逻辑
		ProductService ps = new ProductServiceImpl();
		boolean flag = ps.deleteChecked(pids);
		response.setContentType("text/html;charset=UTF-8");
		if(flag){
	...
	public void search(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//获取客户端传过来的要搜索的pid和pdesc
		String pname = request.getParameter("pname");
		String pdesc = request.getParameter("pdesc");
		//调用业务逻辑
		ProductService ps = new ProductServiceImpl();
		List<Product> list = ps.search(pname,pdesc);
		//将搜索的商品信息设置到域对象中
		request.getSession().setAttribute("list", list);
		//重定向到list.jsp页面
		response.sendRedirect(request.getContextPath()+"/list.jsp");
	...
	public void page(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//调用业务逻辑
		ProductService ps = new ProductServiceImpl();
		PageBean<Product> pb = ps.findPageBean(curPage,pageSize);
