...
<head>
		...
		<script type="text/javascript">
			$(function(){
				...
				$("#img").click(function(){
		 			this.src="${pageContext.request.contextPath}/code?"+new Date();
		 		});
			})
	</script>
...
<div class="row"> 
	...
	<div class="col-md-5">
				...
				<font>会员登录</font>USER LOGIN <font style="color: #DC143C">${msg }</font>
...
<form class="form-horizontal" action="${pageContext.request.contextPath }/user" method="post">
 ...
 <div class="form-group">
   ...
   <div class="form-group">
    ...
    <div class="col-sm-6">
      <input type="password" class="form-control" id="inputPassword3" placeholder="请输入密码" name="password" value="${password }">
    </div>
  </div>
   <div class="form-group">
   ...
   <div class="form-group">
    <div class="col-sm-offset-2 col-sm-10">
      <div class="checkbox">
     	 ...
     	 <%--如果auto的值不为null,那么复选框checkbox的checked就为true --%>
     	 <c:if test="${not empty cookie.info }">
	        <label>
	        	<c:if test="${not empty auto }">
	         	 	<input type="checkbox" name="auto" value="at" checked="checked"> 自动登录
	          	</c:if>
	          	<c:if test="${empty auto }">
	         	 	<input type="checkbox" name="auto" value="at"> 自动登录
	          	</c:if>
	        </label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	        <label>
	          <input type="checkbox" name="remember" value="rem" checked="checked"> 记住用户名
	        </label>
	      </c:if>
	      
	      <%--如果auto的值是null,那么复选框checkbox的checked就为false --%>
	      <c:if test="${empty cookie.info }">
	       	<label>
	        	<input type="checkbox" name="auto" value="at" id="input1"> 自动登录
	        </label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	        <label>
	        	<input type="checkbox" name="remember" value="rem" id="input2"> 记住用户名
	        </label>
	      </c:if>
      </div>
    </div>
