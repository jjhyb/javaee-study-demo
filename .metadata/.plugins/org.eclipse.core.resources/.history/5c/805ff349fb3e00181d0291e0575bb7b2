...
<HTML>
		...
		$(function(){
			$.getJSON("${pageContext.request.contextPath}/category",{method:"findAll"},function(result){
				$(result).each(function(index,element){
					$("#sel").append("<option value="+element.cid+">"+element.cname+"</option>");
				})
			})
		})
	</script>
	...
	<body>
		...
		<form id="userAction_save_do" name="Form1" action="${pageContext.request.contextPath}/upload" method="post" enctype="multipart/form-data">
			&nbsp;
